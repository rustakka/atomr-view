...
`/.github/workflows/release.yml` ships `atomr-view` to three places on
every `v*` tag:

1. **GitHub Releases** — every built Python wheel + sdist attached to
   the release.
2. **crates.io** — `atomr-view-core` then `atomr-view-backends`, in
   dependency order.
3. **PyPI** — platform-specific wheels (Linux x86_64/aarch64 glibc,
   Linux x86_64/aarch64 musl, macOS universal2, Windows x86_64) and
   an sdist.

`atomr-view` does not currently ship any standalone binaries — it is a
library workspace plus a `cdylib` Python extension.
...
| Trigger | verify | wheels | GitHub Release | crates.io | PyPI |
|---|---|---|---|---|---|
| `push` on `v*` tag | ✓ | ✓ | ✓ | ✓ | ✓ |
...
aarch64 Linux wheels are built natively on GitHub-hosted ARM runners
rather than cross-compiled to keep heavy native-dep compilation simple.

| OS | Target | Wheel tag |
|---|---|---|
| Ubuntu | `x86_64-unknown-linux-gnu` | `manylinux_2_17_x86_64` |
| Ubuntu (ARM runner) | `aarch64-unknown-linux-gnu` | `manylinux_2_17_aarch64` |
| Ubuntu | `x86_64-unknown-linux-musl` | `musllinux_1_2_x86_64` |
| Ubuntu (ARM runner) | `aarch64-unknown-linux-musl` | `musllinux_1_2_aarch64` |
| macOS | `universal2-apple-darwin` | `macosx_*_universal2` (fat: x86_64 + arm64) |
| Windows | `x86_64-pc-windows-msvc` | `win_amd64` |

### sdist (`build-sdist`)

A single source distribution `atomr_view-X.Y.Z.tar.gz`, used by PyPI
for platforms that have no pre-built wheel.

### Binaries

None. If a binary is ever added (e.g. a CLI tool for the renderer
backends), introduce a `build-binaries` matrix mirroring atomr's
upstream shape: one matrix entry per target triple, with
`aarch64-unknown-linux-gnu` built via `cross` and the rest built
natively.
...
Workspace members deliberately excluded:

* `xtask` — carries `publish = false`, workspace-internal tooling.
* `atomr-view-python` — `cdylib` only; ships as the `atomr-view` PyPI
  wheel via `maturin`, not an individual crates.io publish.
...
* **Rate limiting** — each successful publish sleeps 30s; `429 Too
  ...
  only 2 crates this caps the publish-crates job around 1 min.
...
This runs the verify gate, builds every wheel, and uploads to TestPyPI
(`https://test.pypi.org/p/atomr-view`) without touching crates.io or
production PyPI. The artifacts also land on the workflow's *Artifacts*
panel so you can download and smoke-test before tagging.
