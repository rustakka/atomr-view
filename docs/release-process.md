...
`atomr-view` release. It covers the day-to-day workflow ("I want to ship
a fix"), the architecture that makes auto-publish work, and the recovery
playbook when something goes sideways.

For workflow-internal detail (job names, matrix entries, build
commands), see [release-pipeline.md](release-pipeline.md). This document
is more pedagogical and self-contained — readable by someone who hasn't
seen the workflows before, or transplantable to other projects with the
same shape.
...
* End-to-end takes ~15 minutes from `git push` to "both Rust crates on
...
This bit upstream `atomr` between v0.6.1 and v0.9.1: five tags landed
in git and zero of them ever ran `release.yml`, so crates.io stayed
frozen at 0.6.0 for four versions. `atomr-view` inherited the same
latent shape from atomr's pre-trampoline workflow and was patched in
2026-05.
...
|---|---|---|
| 2 Rust crates (`atomr-view-core`, `atomr-view-backends`) | crates.io | `publish-crates` job, sequentially in dep order |
...
| Release notes + wheel/sdist attachments | GitHub Release | `github-release` (auto-generated from commit subjects) |

Wheel ABI: cp310-abi3 (one wheel per platform covers Python 3.10–3.13
via the stable ABI).

No binaries are produced — `atomr-view` is a library workspace plus a
Python extension. If a binary ever becomes part of the artifact set,
add a `build-binaries` matrix to `release.yml` mirroring atomr's
upstream shape.

## Crate publish order

The `publish-crates` job walks the two publishable crates strictly in
dep-order with a 30s throttle between successful publishes (to respect
crates.io rate limits) and exponential backoff on `429 Too Many Requests`.

```
Layer  Crate
─────  ──────────────────────────────────────────────
  1    atomr-view-core
  2    atomr-view-backends
```

`atomr-view-python` is a `cdylib` only — it ships as a PyPI wheel via
`maturin`, not as a crates.io publish. `xtask` is workspace-internal
...
2. In the new crate's `Cargo.toml`, declare every intra-workspace dep
   ...
   release bugs.
3. Slot the crate into the **earliest** layer of the `for crate in ...`
   loop in `release.yml`'s `publish-crates` job whose prior layers have
   published all of its deps.
...
`atomr-view` consumes upstream crates from sibling repos via
`[workspace.dependencies]` with `{ git = ..., version = "X.Y" }`. The
git source keeps local builds reproducible; the `version` field is what
`cargo publish` records for downstream consumers resolving from
crates.io.

Current pins (as of 2026-05-11):

| Crate | Pin | Upstream repo |
|---|---|---|
| `atomr-core` | `0.9` | `rustakka/atomr` |
| `atomr-persistence` | `0.9` | `rustakka/atomr` |
| `atomr-infer` | `0.8` | `rustakka/atomr-infer` |

Only `atomr-core` is actively consumed today; `atomr-persistence` and
`atomr-infer` are declared for future use. When bumping pins, run
`cargo build --workspace` and `cargo test --workspace` afterwards —
0.x crates break API freely across minor versions.

## PyPI publishing

`atomr-view` ships as a single PyPI distribution (the project name is
`atomr-view`), built by [`maturin`](https://www.maturin.rs/) from the
`crates/atomr-view-python` crate.

Authentication uses [PyPI Trusted Publishing](https://docs.pypi.org/trusted-publishers/),
not an API token. The OIDC handshake between GitHub Actions and PyPI
...
Runs the verify gate, builds every wheel, runs `cargo publish
--dry-run` on both crates, and uploads to TestPyPI. Skips crates.io
...
### "tag exists but nothing published"

Root cause: the tag was created by `version-bump.yml` (using the
default `GITHUB_TOKEN`) without the trampoline step, so `release.yml`
never fired.

Recovery: cut a fresh release using a `fix:` commit (or
`Release-As: <next-version>` footer). The new pipeline takes over
and both crates publish at the new version, jumping over the
unpublished tag. Crates.io enforces monotonically increasing
versions, so leapfrogging is fine and supported.

### "publish-crates failed mid-loop"
...
### "failed to select a version for the requirement `atomr-view-foo = ^X.Y.Z`"
...
* You add a new artifact target (new wheel platform, binary OS).
* You discover a new failure mode worth a troubleshooting entry.
* You bump upstream pins (`atomr-core`, `atomr-persistence`,
  `atomr-infer`) — update the [Upstream dependency pins](#upstream-dependency-pins)
  table.
